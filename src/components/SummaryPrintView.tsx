...
// A4 at 96dpi = 794px wide, padding 40px each side → content 714px.
// Two-column: (714 - 16 gap) / 2 = 349px per col. Card padding 16px → chart 317px.
const CONTENT_W   = 714;
const COL_W       = 349;
const CHART_W     = COL_W - 32;   // 317
const CHART_BAR_H = 190;
const CHART_AREA_H = 160;

// ── Page-break sentinel height — cuts happen here, not mid-element ─────────
// We add a tall white gap between major sections so the slicer always lands
// in white space between sections rather than through an element.
const BREAK_GAP = 40;
...
function periodLabel(dateFrom: Date, dateTo: Date): string {
  ...
  if (days === 28 || days === 30) return "Last 30 days";
...
function buildSummary(brand: string, ga4: any, gsc: any, dateFrom: Date, dateTo: Date): string {
  ...
  if (gsc?.totalImpressions > 0) {
    ...
    parts.push(
      `Search visibility: ${fmt(gsc.totalImpressions)} impressions, ${fmt(gsc.totalClicks)} clicks` +
      (ctr ? ` (${ctr.toFixed(1)}% CTR)` : "") +
      (gsc.averagePosition ? `, avg. position #${gsc.averagePosition.toFixed(1)}` : "") +
      "."
    );
  }
  if (!parts.length)
    return `Performance report for ${brand} — ${format(dateFrom, "MMM d")}–${format(dateTo, "MMM d, yyyy")}.`;
  return parts.join(" ");
}
...
function SectionHeader({ label, source }: { label: string; source?: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderLeft: `3px solid ${RED}`, paddingLeft: 10, marginBottom: 12 }}>
      <span style={{ fontSize: 10, fontWeight: 900, textTransform: "uppercase" as const, letterSpacing: "0.15em", color: BLACK, fontFamily: FONT }}>{label}</span>
      {source && <span style={{ fontSize: 10, color: MUTED, fontFamily: FONT }}>{source}</span>}
    </div>
  );
}

function KpiTile({ label, value, delta, sub, invertDelta, last }: {
      ...
      {top.map((ch) => {
        ...
        return (
          <div key={ch.channel} style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ flex: 1, height: 13, backgroundColor: BORDER, borderRadius: 2, overflow: "hidden" }}>
              <div style={{ width: `${Math.max(pct, 1)}%`, height: "100%", backgroundColor: color }} />
            </div>
            <div style={{ width: 50, textAlign: "right", fontSize: 10, fontWeight: 700, fontFamily: FONT, color: BLACK, fontVariantNumeric: "tabular-nums" }}>
              {v.toLocaleString()}
            </div>
          </div>
        );
  ...
  return (
    <div style={{ width: 794, backgroundColor: "#fff", fontFamily: FONT, padding: "32px 40px", boxSizing: "border-box", color: BLACK }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: MUTED, marginTop: 5, fontFamily: FONT }}>
              {format(dateFrom, "MMM d")} – {format(dateTo, "MMM d, yyyy")} · {period}
            </div>
          </div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: MUTED, fontFamily: FONT, paddingTop: 4, marginBottom: 20 }}>
        ...
        <span>{[brand.hasGA4 && "Google Analytics 4", brand.hasGSC && "Search Console", brand.hasHubSpot && "HubSpot"].filter(Boolean).join(" · ")}</span>
        ...
        <>
          <div style={{ display: "flex", gap: 16 }}>
            {ga4?.sessionsOverTime?.length > 0 && (
              <div style={{ ...card, width: COL_W, flexShrink: 0, padding: "12px 16px" }}>
                ...
                <BarChart width={CHART_W} height={CHART_BAR_H} data={ga4.sessionsOverTime} margin={{ top: 4, right: 4, left: -22, bottom: 0 }}>
            ...
            {channels.length > 0 && (
              <div style={{ ...card, flex: 1, padding: "12px 16px" }}>
                <div style={{ fontSize: 9, fontWeight: 900, textTransform: "uppercase" as const, letterSpacing: "0.15em", color: BLACK, fontFamily: FONT, marginBottom: 3 }}>
                  ...
                  <span style={{ fontWeight: 400, color: MUTED, textTransform: "none" as const, letterSpacing: "normal" }}>(Users)</span>
                </div>
        ...
        <>
          ...
          {gsc.clicksImpressionsOverTime?.length > 0 && (
            <div style={{ ...card, padding: "12px 16px", marginBottom: 16 }}>
              ...
              <AreaChart width={CONTENT_W - 32} height={CHART_AREA_H} data={gsc.clicksImpressionsOverTime} margin={{ top: 4, right: 4, left: -22, bottom: 0 }}>
                <defs>
            ...
            {recommendations.slice(0, 5).map((rec: any, i: number) => {
              ...
              return (
                <div key={rec.id} style={{ border: `1px solid ${BORDER}`, borderLeft: `4px solid ${lc}`, borderRadius: 4, backgroundColor: CARD_BG, padding: "11px 14px", display: "flex", gap: 12, alignItems: "flex-start" }}>
                  <div style={{ width: 20, height: 20, borderRadius: "50%", backgroundColor: BLACK, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 900, fontFamily: FONT, flexShrink: 0 }}>
                    {i + 1}
                  </div>
                  <div>
